...
function InterviewCard({ interview,viewDetail=false}) {
      const url="https://localhost:3000/interview"+ "/" +interview?.interview_id
    ...
    const onSend=()=>{
        window.location.href="mailto:account@anchal.chaurasia05@gmail.com?subject=AiCruiter Interview Link & body=Interview Link:"+ url
    }
  return (
  ...
  <span className='text-green-700'>
    {(interview?.['interview-feedback']?.length ?? 0)} Candidates
        ...
        {!viewDetail? <div className='flex gap-3 w-full mt-5'>
            <Button variant='outline' className={'w-small'} onClick={copyLink}> <Copy /> Copy Link</Button>
            <Button className={'w-small'} onClick={onSend}> <Send /> </Button>
        </div>
        :
